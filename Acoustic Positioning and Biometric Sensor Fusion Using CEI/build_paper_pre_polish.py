"""
Paper 2 — Day 1 build, Block 1 Tasks 1-4 applied.
Reconstructed from uploaded PDF.

Tasks integrated in this build:
  T1. Missing keywords surfaced in abstract:
      - "personnel recovery" added
      - "oscillation suppression" added (replaces "oscillation reduction" in one place)
      - "governance-aware allocation" surfaced verbatim
      - "UUV" surfaced explicitly
  T2. Explicit research question statement added to Introduction (paragraph 4 head)
  T3. Roadmap paragraph added at end of Introduction
  T4. Explicit body citations added for every table (Tables I-IX)
"""
...
OUT = "/home/claude/paper2_day1/paper2_revised_v8_camera_ready.pdf"
...
equation_style = ParagraphStyle('Eq', parent=styles['Normal'],
    ...
    spaceBefore=4, fontName='Times-Roman')
...
fig_caption = ParagraphStyle('FigCaption', parent=styles['Normal'],
    ...
    spaceBefore=4, fontName='Times-Bold')
...
def build():
    doc = SimpleDocTemplate(OUT, pagesize=A4,
        ...
        title="Paper 2 - Sensor Networks (CEI) - LNNS Camera-Ready")
    story = []
    ...
    story.append(Paragraph(
        ...
        "fusion for personnel recovery share a fundamental physics "
        "constraint: severe signal attenuation through water and obstacles "
        "creates bandwidth scarcity that static allocation wastes and "
        "reactive allocation mismanages. This paper introduces the "
        "Centrality-Entropy Index (CEI) framework, a governance-aware "
        "allocation methodology that jointly integrates centrality, "
        "entropy, and governance constraints into a unified resource "
        "allocation framework for these environments. We derive three "
        "formal results: an oscillation frequency bound, a conditional "
        "availability guarantee, and an algorithm complexity "
        "characterization. Thorp-model simulation of a 12-node underwater "
        "acoustic network demonstrates 50% oscillation suppression versus "
        "reactive baselines while maintaining 100% positioning "
        "availability and 24.2% bandwidth savings. Simulation of a 24-node "
        "multi-modal sensor fusion network achieves 95.6% detection "
        "pipeline availability under coordinated jamming versus 76.8% for "
        "threshold baselines, with 41% reduction in wasted bandwidth. CEI "
        "maintains governance compliance above 97% in both domains. We "
        "further compare CEI against two reinforcement-learning baselines: "
        "unconstrained PPO matches CEI in benign stationary regimes but "
        "exhibits a 19.8 percentage-point governance compliance drop under "
        "coordinated jamming, while Lagrangian-PPO restores governance "
        "only by inducing substantial wasted bandwidth (approximately 90% "
        "under the jamming scenario). CEI uniquely achieves both "
        "governance compliance and bandwidth efficiency under adversarial "
        "conditions.",
        abstract_body))
    ...
    story.append(Paragraph(
        ...
        "framework across two operationally distinct domains with five "
        "contributions. First, we adapt the CEI framework for sensor "
        "networks with three formal propositions. Second, we present an "
        ...
        "domains. Fifth, we align the work with DARPA ANS, POSYDON, and "
    ...
    story.append(Paragraph(
        ...
        "entropy-conditioned allocation rule. Section 5 derives the three "
        "theoretical propositions, clarifies their epistemic status, and "
        "presents an empirical validation of the oscillation bound. "
    ...
    story.append(Paragraph(
        ...
        equation_style))
    story.append(Paragraph(
        "where C<sub>i</sub> is normalized betweenness centrality [25] over "
        "links exceeding quality threshold, H<sub>i</sub> = "
        "\u2013&Sigma; p<sub>k</sub> log<sub>2</sub>(p<sub>k</sub>) is "
        "Shannon entropy [24], [26] of the discretized quality distribution "
        "over a W-slot window, and R<sub>i</sub> encodes governance "
        "priority. Weights satisfy &alpha;+&beta;+&gamma;=1.",
        body))
    ...
    story.append(Paragraph(
        ...
        equation_style))
    story.append(Paragraph(
        "with &lambda;<sub>1</sub>=0.7, &lambda;<sub>2</sub>=0.3. Weights "
        "updated via projected gradient descent [21], [22] on L over the "
        "probability simplex. Default: &alpha;=0.4, &beta;=0.2, "
        "&gamma;=0.4.",
        body))
    ...
    story.append(Paragraph(
        ...
        "Proposition 3 (complexity) is a standard algorithm analysis. The "
        "simulation results in Sections 6 and 7 are empirical findings "
        "under specific channel models and parameter choices. They "
        "demonstrate that CEI improves allocation quality relative to "
        "baselines in the tested scenarios. However, they do not "
        "constitute proof of superiority in general sensor network "
        "deployments. The Thorp-model underwater channel and the "
        "synthetic sensor fusion network are reasonable approximations "
        "for applied research but do not substitute for operational "
        "hardware validation.",
        body))
    ...
    story.append(Paragraph(
        ...
        "its entropy-conditioned rule. CEI uniquely achieves both "
        "governance compliance (97.3%) and bandwidth efficiency "
        "(18.7% waste) under jamming, demonstrating that the integration "
    ...
    story.append(Paragraph(
        ...
        "PPO matches or exceeds CEI on benign scenarios but exhibits "
        "governance compliance collapse under coordinated jamming "
        "(77.5% versus CEI's 97.3% in S3), while constrained-RL "
        "(Lagrangian-PPO) restores governance only by inducing severe "
        "bandwidth waste (approximately 90% under S3). CEI uniquely "
        "achieves both governance compliance and bandwidth efficiency "
        "across all evaluated adversarial conditions.",
        body))
    ...
    story.append(Paragraph(
        ...
        "Three formal results: oscillation bound, availability guarantee, "
        "complexity characterization. Underwater: 50% oscillation "
        "reduction, 100% availability, 24.2% BW savings. Sensor fusion: "
        "95.6% availability under jamming (vs 76.8% threshold), 41% BW "
        "waste reduction. Governance exceeded 97% in both domains. We "
    ...
    refs = [
        '[1] DARPA, "Adaptable Navigation Systems (ANS)," Strategic Technology Office.',
        '[2] DARPA, "Positioning System for Deep Ocean Navigation (POSYDON)," Strategic Technology Office.',
        '[3] Draper Laboratory, "Improving Stealth Capabilities for Undersea Vehicles," May 2016.',
        '[4] U.S. Navy FY2025 Budget: UUV Family of Systems, $191.5M. CRS R45757, Mar. 2025.',
        '[5] GAO-22-105974, "Extra Large UUV: Better Management Practices Needed," Sept. 2022.',
        '[6] X. Liang et al., "UWB Impulse Radar Through-Wall Detection of Vital Signs," Sci. Rep., vol. 8, 2018.',
        '[7] C. L. Degen et al., "Quantum Sensing," Rev. Mod. Phys., vol. 89, art. 035002, 2017.',
        '[8] J. F. Barry et al., "Sensitivity Optimization for NV-Diamond Magnetometry," Rev. Mod. Phys., vol. 92, 2020.',
        '[9] Z. Du et al., "Widefield Diamond Quantum Sensing," Adv. Sci., vol. 11, 2024.',
        '[10] DARPA, "SIGMA Program: Networked Sensors for Threat Detection."',
        '[11] J. Heidemann et al., "Underwater Sensor Networking," IEEE WCNC, 2006.',
        '[12] R. Otnes et al., "Underwater Acoustic Wireless Sensor Networks," Sensors, vol. 14, 2014.',
        '[13] R. Santos et al., "Real-Time Communication for Underwater Sensors," Sensors, vol. 17, 2017.',
        '[14] U.S. DoD CDAO, "2023 DoD Data, Analytics, and AI Adoption Strategy," Nov. 2023.',
        '[15] I. F. Akyildiz et al., "Underwater Acoustic Sensor Networks," Ad Hoc Networks, vol. 3, 2005.',
        '[16] DSIAC, "Research Efforts in Submarine Acoustic Communications," 2021.',
        '[17] M. G. Amin, Ed., Through-the-Wall Radar Imaging. CRC Press, 2011.',
        '[18] T. Wolf et al., "Subpicotesla Diamond Magnetometry," Physical Review X, vol. 5, art. 041001, 2015.',
        '[19] U. K. Lilhore et al., "Depth-Controlled Routing for Underwater WSN," IJDSN, vol. 18, 2022.',
        '[20] A. Deng and B. Hooi, "GNN Anomaly Detection," AAAI, 2021.',
        '[21] S. Boyd and L. Vandenberghe, Convex Optimization. Cambridge, 2004.',
        '[22] D. P. Bertsekas, Nonlinear Programming, 3rd ed. Athena Scientific, 2016.',
        '[23] R. Albert et al., "Error and Attack Tolerance of Complex Networks," Nature, vol. 406, 2000.',
        '[24] C. E. Shannon, "A Mathematical Theory of Communication," Bell Sys. Tech. J., vol. 27, 1948.',
        '[25] L. C. Freeman, "Centrality in Social Networks," Social Networks, vol. 1, 1979.',
        '[26] T. M. Cover and J. A. Thomas, Elements of Information Theory, 2nd ed. Wiley, 2006.',
        '[27] U.S. DoD, "2022 National Defense Strategy," Oct. 2022.',
        '[28] T. Lorido-Botran et al., "Auto-scaling Techniques Review," J. Grid Comp., vol. 12, 2014.',
        '[29] D. J. Watts and S. H. Strogatz, "Small-World Networks," Nature, vol. 393, 1998.',
        '[30] J. Schulman et al., "Proximal Policy Optimization Algorithms," arXiv:1707.06347, 2017.',
        '[31] P. Pokharel, "Governance-Aware Resource Allocation for Defense Infrastructure Using the Centrality-Entropy Index," FTC 2026 (companion paper, in press).',
        '[32] J. Achiam et al., "Constrained Policy Optimization," ICML, 2017.',
    ]
    for r in refs:
